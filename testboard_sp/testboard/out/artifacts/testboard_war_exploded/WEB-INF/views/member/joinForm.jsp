    ...
    <script>
        ...
        function checkId() {
            ...
            callService(
                ...
                , "/member/check/id"
                , {
                    loginId: input //key값만 같으면 보내짐 getParameter x (->IdCheckDTO loginId에 들어감)
                }
                , function (response) {
            ...
            callFormService(
                ...
                , function (response) {
                    if (response.isSuccess) {
                        location.replace("/member/join/result"); //replace라서 memberController에서 GET으로 함
                    } else {                                     // replace쓰면 뒤로갈때 데이터 안남음
                        alert(response.message);
                    }
                }
            );
