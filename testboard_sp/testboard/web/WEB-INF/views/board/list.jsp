...
<head>
    ...
    <script>
        ...
        function gotoList() {
            ...
            location.href = '/board/list?page=${mc.page}' + '&category=${mc.category}&keyword=${mc.keyword}&sort=' + selected;
        }

        // 페이지 이동
        function gotoPage(pageNo) {
            location.href = '/board/list?page=' + pageNo + '&category=${mc.category}&keyword=${mc.keyword}&sort=${mc.sort}';
        }

        function gotoWriteForm() {
            location.href = '/board/write?page=${mc.page}' + '&category=${mc.category}&keyword=${mc.keyword}&sort=${mc.sort}';
        }
...
<table>
    ...
    <%--실제 글 목록--%>
    <%--row에 list의 값이 하나하나 들어온다--%>
    <c:choose>
        <c:when test="${fn:length(mc.list) > 0}">
            <c:forEach items="${mc.list}" var="row"> <!--list=>model에 넣었던 key값-->
                <tr class="article" data-aid="${row.id}">
                    <td>${row.title}(${row.replyCount})</td>
                    <td>${row.hits}</td>
                    <td>${row.registerLoginId}</td>
                    <td>${row.registerDatetime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd hh:mm"))}</td>
                </tr>
            </c:forEach>
        </c:when>
        <c:otherwise>
